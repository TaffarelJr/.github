...
<#
.SYNOPSIS
    Create a new TEMPLATE repo (.template-<type>) derived from THIS template repo.
...
.PARAMETER Type
    Template type (kebab-case), e.g. 'dotnet' -> creates '.template-dotnet'.
.PARAMETER GhAccount
...
.EXAMPLE
    ./scripts/New-TemplateRepo.ps1                       # fully interactive
.EXAMPLE
    ./scripts/New-TemplateRepo.ps1 -Type dotnet          # prompts only for the rest
.EXAMPLE
    ./scripts/New-TemplateRepo.ps1 -Type dotnet -GhAccount TaffarelJr `
        -Description 'A .NET template' -Homepage '' -Topics 'dotnet, template-repository' `
...
$Type = Resolve-ScaffoldValue -Name Type -Bound $bound -Value $Type -Prompt "Template type (kebab-case, e.g. 'dotnet' -> '.template-dotnet')"
$Type = Format-ScaffoldSlug -Value $Type -Label 'Type'
...
Write-ScaffoldField 'New template'    $ownerRepo
...
Invoke-ScaffoldGatedCommit -RepoPath $targetPath -TemplateBranch $TemplateBranch `
    ...
    Write-ScaffoldSettings -RepoPath $targetPath -Kind Template -Name $repo `
...
if ((Get-ScaffoldActivity) -eq 0) {
    ...
    Write-Host "  🎉 Template repo $ownerRepo ready at $targetPath." -ForegroundColor Green
}
Write-Host ""
