...
- a rename or restructure with no behavior change,
- formatting only,
- docs only,
- tests only.

Never sweep unrelated uncommitted work into a commit to save a step. If you
find changes you don't recognize, leave them alone and say so.
...
- Add a body when the change needs a *why* — the constraint you were working
  ...
  reveal on its own. A few lines, wrapped at 72 characters. If it runs past
  a short paragraph, it is describing the diff instead of explaining it.
