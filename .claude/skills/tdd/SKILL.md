...
Work in one-behavior cycles. Never write production code without a failing
test demanding it.

## Before the first cycle

State the behavior in one sentence, and list the cases you expect to cover —
...
Then stop and report the cycle before starting the next one. One behavior
per cycle, and never two failing tests at once.

## Test quality

- Assert **behavior**, not implementation. A test that restates the code
...
doesn't reach. `.github/codecov.yml` sets the coverage the repo enforces on
changed lines; say plainly what you left uncovered and why, and never write a
test purely to move the number.
