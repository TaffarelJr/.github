...
Install the [Editor Guidelines][guidelines] extension.
The rulers are already defined in [`.editorconfig`][editorConfig].

### Visual Studio Code

The rulers are already defined in [`settings.json`][vsCodeSettings].
...
For Vim or Neovim, add the following to your `.vimrc` or `init.vim`:
...
you can use a plugin like [vim-signature][vim-signature]
